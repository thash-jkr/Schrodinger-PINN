...
class PINN(nn.Module):
    ...
    def loss_function(self, x_collocation_torch, t_collocation_torch, x_initial_torch, t_initial_torch, x_boundary_torch, t_boundary_torch):
        #pde loss
...
# Model setup
model = PINN(layers, t_min, t_max).to(device)
...
history = model.train_model(optimizer, scheduler, 150000)

torch.save(model.state_dict(), "Schrodinger-PINN/src/results/baseline/model_1.pth")

with open("Schrodinger-PINN/src/results/baseline/history_1.json", "w") as f:
    json.dump(history, f)
