...
class PINN(nn.Module):
    ...
    def generator(self, T_min, T_max):
        ...
        omega_collocation = np.random.uniform(1 / hbar, 2 / hbar, self.n_collocation)
    
        x_initial = np.random.normal(loc=0, scale=25.0, size=self.n_initial)
        t_initial = np.full(self.n_initial, T_min)
        omega_initial = np.random.uniform(1 / hbar, 2 / hbar, self.n_initial)
        ...
        omega_boundary = np.random.uniform(1 / hbar, 2 / hbar, self.n_boundary)
        ...
        omega_collocation_torch = torch.from_numpy(omega_collocation).float().to(device)
        
        x_initial_torch  = torch.from_numpy(x_initial).float().to(device)
        t_initial_torch  = torch.from_numpy(t_initial).float().to(device)
        omega_initial_torch = torch.from_numpy(omega_initial).float().to(device)
        
        x_boundary_torch = torch.from_numpy(x_boundary).float().to(device)
        t_boundary_torch = torch.from_numpy(t_boundary).float().to(device)
        omega_boundary_torch = torch.from_numpy(omega_boundary).float().to(device)
    
        return x_collocation_torch, t_collocation_torch, omega_collocation_torch, x_initial_torch, t_initial_torch, omega_initial_torch, x_boundary_torch, t_boundary_torch, omega_boundary_torch

    def loss_function(self, x_collocation_torch, t_collocation_torch, omega_collocation_torch, x_initial_torch, t_initial_torch, omega_initial_torch, x_boundary_torch, t_boundary_torch, omega_boundary_torch):
        #pde loss
        ...
        u, v = self((x_collocation_torch, t_collocation_torch, omega_collocation_torch))
        ...
        u_i, v_i = self((x_initial_torch, t_initial_torch, omega_initial_torch))
        
        psi_initial_real = (((m * omega_initial_torch) / (np.pi * hbar)) ** 0.25) * torch.exp(((-m * omega_initial_torch) / (2 * hbar)) * (x_initial_torch ** 2))
        initial_condition_loss = torch.mean((u_i - psi_initial_real) ** 2) + torch.mean((v_i - 0) ** 2)
        
        
        
        
        #boundary condition loss
        u_b, v_b = self((x_boundary_torch, t_boundary_torch, omega_boundary_torch))
    ...
    def train_model(self, optimizer, scheduler, epochs):
        history = []
        
        for epoch in range(1, epochs+1):
            optimizer.zero_grad()
            
            physics_loss, initial_condition_loss, boundary_condition_loss = self.loss_function(*self.generator(self.t_min, self.t_max))
